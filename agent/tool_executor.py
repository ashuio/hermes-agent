...
def _commit_tool_result(
    ...
    *,
    ...
    # TARS-PATCH: proactive promotion for routes that reject or silently STRIP tool-role
    # images. Cheap gates first: only image-bearing results for vision-capable models pay
    # for the (cached) route resolution below. Bifrost strips silently (HTTP 200 — no 400
    # ever fires, so the reactive recovery in turn_recovery.py never triggers); a promoted
    # user message is the one shape every provider tested accepts.
    # NOTE: agent.provider is the CANONICALIZED name ('custom') — resolution matches
    # custom_providers entries by base_url too, and (provider, model) pairs recorded by
    # reactive recovery promote pre-emptively via _promote_tool_content_models
    # (agent/vision_tool_veto.py). computer_use is excluded deliberately: capture routing
    # pre-decides aux-vs-envelope itself (tools/computer_use/vision_routing.py), and when
    # it chooses the envelope the provider already accepts tool-result images.
    if (_is_multimodal_tool_result(persisted_result)
        ...
        try:
            ...
            _pm_key = _provider_model_key(agent)
            _needs_promotion = bool(
                _pm_key[1]
                and _pm_key in (getattr(agent, "_promote_tool_content_models", None) or ())
            )
            if not _needs_promotion:
                from agent.vision_tool_veto import resolve_tool_result_image_veto
                _needs_promotion = resolve_tool_result_image_veto(
